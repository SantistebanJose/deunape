...
<script>
  ...
  function fn_agregar_venta(articulo) {
    ...
    let precios = [];
    try {
      precios = typeof articulo.precios_json === 'string' ?
        JSON.parse(articulo.precios_json) :
        (articulo.precios_json || []);
    } catch (e) {
      precios = [];
    }
    ...
    if (precios.length > 0) {
      ...
      precios.forEach(p => {
        ...
        pill.innerHTML = `<span class="pill-nombre">${nombreUnidad}</span>
                              <span class="pill-precio">S/ ${parseFloat(p.precio).toFixed(2)}</span>
                              <span style="font-size:.68rem;opacity:.7;">(${cantUnidad} unid.)</span>`;
    ...
    document.getElementById('btnConfirmarCantidad').onclick = () => {
      ...
      // Precio unitario real = precio presentación / unidades presentación
      ...
      const nombreFinal = articulo.articulo +
        (nombrePresentacionSeleccionada !== 'Unidad' ? ` [${nombrePresentacionSeleccionada}]` : '') +
        (nota ? ` - ${nota}` : '');
          ...
          if (qtyEl) {
            ...
            if (nueva > stockGuardado) {
              Swal.fire({
                icon: 'warning',
                title: 'Stock insuficiente',
                html: `Solo hay <b>${stockGuardado}</b> unidades disponibles.<br>Ya tienes <b>${actual}</b> en la reserva.`,
                timer: 2500,
                showConfirmButton: false
              });
              return;
    ...
    document.getElementById('btnConfirmarReserva').addEventListener('click', () => {
      ...
      if (idCliente === '#') {
        Swal.fire({
          ...
          text: 'Selecciona o registra un cliente para la reserva',
          timer: 2000,
          showConfirmButton: false
        });
          ...
          try {
            ...
            if (res.success) {
              Swal.fire({
                  ...
                  text: 'La reserva quedó pendiente de pago.',
                  timer: 1500,
                  showConfirmButton: false
                })
              ...
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: res.message || 'No se pudo registrar la reserva'
              });
            }
          } catch (e) {
            Swal.fire({
