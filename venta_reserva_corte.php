...
#panelBuscar{position:relative;overflow:visible;}

/* === BADGE RESERVA en resumen === */
.reserva-badge{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:white;border-radius:10px;padding:8px 14px;font-size:.78rem;font-weight:700;text-align:center;margin-bottom:10px;display:flex;align-items:center;justify-content:center;gap:6px;}
    ...
    datos.forEach(item => {
        ...
        if (cantidadEsNumero) {
            ...
            btnPlus.addEventListener('click', () => {
                ...
                if(v>=stockActual){Swal.fire({icon:'warning',title:'Stock máximo alcanzado',text:`Solo hay ${stockActual} unidades disponibles.`,timer:2000,showConfirmButton:false});return;}
...
function limpiarVenta() {
    ...
    Swal.fire({title:'¿Limpiar reserva?',text:'Se eliminarán todos los artículos.',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc3545',cancelButtonColor:'#6c757d',confirmButtonText:'Sí, limpiar',cancelButtonText:'Cancelar'}).then(r=>{if(r.isConfirmed){document.querySelector('#tabla_articulos tbody').innerHTML='';calcularTotales();}});
}
...
function initReservaModal() {
    document.getElementById('btnConfirmarReserva').addEventListener('click', () => {
        ...
        btnR.disabled = true; btnR.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Procesando...';
        $.ajax({
            method: 'POST',
            url: 'logica/clssVentaCorte.php',
            data: { accion: 'REGISTRARRESERVA', data: JSON.stringify(datos) },
            success: r => {
                try {
                ...
                } catch(e) { Swal.fire({icon:'error',title:'Error',text:'Error al procesar la respuesta'}); }
                btnR.disabled = false; btnR.innerHTML = '<i class="fas fa-bookmark me-2"></i>Registrar Reserva';
            },
            error: () => {
...
function buscarCliente(){const q=document.getElementById('nombreCliente').value.trim();const s=document.getElementById('sugerencias');s.innerHTML='';if(!q.length)return;$.ajax({method:'POST',url:'logica/clssFiltro.php',data:{accion:'FILTROPERSONA',data:q,sucursal_id:<?php echo $sucursal_id;?>},success:r=>{try{const arr=JSON.parse(r);arr.forEach(p=>{const item=document.createElement('div');item.className='list-group-item list-group-item-action';item.textContent=p.persona_concatenada;item.onclick=()=>{document.getElementById('nombreCliente').value=p.persona_concatenada;document.getElementById('idPersona').textContent=p.id;s.innerHTML='';};s.appendChild(item);});}catch(e){}}}); }
...
function initEventListeners(){initSoloCorteModal();initImpresion3DModal();initReservaModal();initClienteModal();}
</script>
