...
/* === TABLA === */
#tabla_articulos{width:100%;border-collapse:collapse;}
#tabla_articulos thead tr{background:var(--primary);color:white;}
#tabla_articulos thead th{padding:11px 14px;font-size:.82rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap;}
#tabla_articulos thead th:first-child{border-radius:8px 0 0 8px;}
#tabla_articulos thead th:last-child{border-radius:0 8px 8px 0;}
#tabla_articulos tbody tr{border-bottom:1px solid #f0f3ff;transition:background .15s;}
...
.modal-header-success .modal-title{font-weight:800;}
...
.pago-row{background:var(--bg-card);border:1.5px solid var(--border-soft);border-radius:12px;padding:12px 16px;margin-bottom:10px;}
#seccionCorte{background:var(--bg-card);border:1.5px solid var(--border-soft);border-radius:14px;padding:16px;margin-top:12px;}
...
/* === TABS MODALES === */
.nav-venta .nav-link{border-radius:10px;font-weight:600;font-size:.88rem;color:var(--text-muted);padding:8px 18px;transition:all .2s;border:none;}
.nav-venta .nav-link.active{background:var(--gradient-main);color:white;box-shadow:0 3px 10px rgba(102,126,234,.3);}
.nav-venta .nav-link:not(.active):hover{background:#f0f3ff;color:var(--primary);}
...
@media(max-width:768px){
    ...
    .venta-header::before{display:none;}
    #tabla_articulos thead{display:none;}
    #tabla_articulos tbody td{display:block;text-align:right;padding:6px 12px;}
    #tabla_articulos tbody td::before{content:attr(data-label);float:left;font-weight:600;color:var(--text-muted);font-size:.8rem;}
    #tabla_articulos tbody tr{margin-bottom:12px;border:1.5px solid var(--border-soft);border-radius:10px;}
}
</style>
        ...
        <div class="tab-content">
          <div class="tab-pane fade show active" id="pills-persona">
            <div class="mb-3"><label class="fw-bold" style="font-size:.83rem;color:var(--primary);">DNI <span class="text-danger">*</span></label>
              ...
              <button class="btn" type="button" id="btnBuscarDNI" style="background:var(--gradient-main);color:white;border:none;border-radius:0 10px 10px 0;"><i class="fas fa-search"></i></button></div></div>
          ...
          <div class="tab-pane fade" id="pills-empresa">
            <div class="mb-3"><label class="fw-bold" style="font-size:.83rem;color:var(--primary);">RUC <span class="text-danger">*</span></label>
              ...
              <button class="btn" type="button" id="btnBuscarRUC" style="background:var(--gradient-main);color:white;border:none;border-radius:0 10px 10px 0;"><i class="fas fa-search"></i></button></div></div>
...
function checkCotizacion() {
    ...
    try {
        ...
        if (cot.cliente) { document.getElementById('nombreCliente').value = cot.cliente.persona_concatenada; document.getElementById('idPersona').textContent = cot.cliente.id; }
        cot.items.forEach(i => agregarATabla([{id:i.productoId||'0',articulo:i.descripcion,cantidad:i.cantidad,precio_unitario:i.precioUnit,subtotal:i.cantidad*i.precioUnit,idmovimiento:1,nota:i.nota||''}]));
        Swal.fire({icon:'success',title:'¡Cotización cargada!',html:`<strong>${cot.codigo}</strong> — ${cot.items.length} artículos precargados`,timer:2500,showConfirmButton:false});
    } catch(e) {}
}
...
function filterProducts() {
    ...
    if (!q&&!cat&&!tip&&!dim&&!col) { box.style.display='none'; if(hint) hint.style.display='block'; return; }
    ...
    box.innerHTML='';
    if (!filtered.length) { box.innerHTML='<div class="producto-item"><span class="prod-info">Sin resultados</span></div>'; box.style.display='block'; return; }
    filtered.slice(0,15).forEach(p=>{
        ...
        if (!sinStock) { div.addEventListener('click',()=>fn_agregar_venta(p)); }
        else { div.style.opacity='.5'; div.style.cursor='default'; }
        box.appendChild(div);
    });
...
function agregarManual() {
    ...
    if(!desc){Swal.fire({icon:'warning',title:'Falta descripción',text:'Ingresa el nombre del artículo.',confirmButtonText:'Ok'});return;}
    agregarATabla([{id:'0',articulo:desc,cantidad:cant,precio_unitario:precio,subtotal:cant*precio,idmovimiento:1,nota:''}]);
    document.getElementById('manualDescripcion').value=''; document.getElementById('manualCantidad').value=1; document.getElementById('manualPrecio').value='';
...
function fn_agregar_venta(articulo) {
    ...
    document.getElementById('btnRestarCantidad').onclick=()=>{let c=parseInt(document.getElementById('inputCantidad').value);if(c>1){document.getElementById('inputCantidad').value=c-1;if(c-1===1&&articulo.corte){sc.style.display='block';document.getElementById('precioCorte').value=1.5;}else if(c-1>1)sc.style.display='none';}};
...
function calcularTotales() {
    ...
    el.classList.remove('pulse-total'); void el.offsetWidth; el.classList.add('pulse-total');
...
function initVueltoListeners() {
    ...
    document.getElementById('montoTotalFinal').addEventListener('input',()=>{if(document.getElementById('seccionVuelto').style.display!=='none')actualizarTotalAPagar();});
}
function detectarEfectivo(){
    let hay=false;
    const sp=document.getElementById('formaPagoSelect'); const mp=parseFloat(document.getElementById('montoSelect_0').value)||0;
    if(mp>0&&sp.options[sp.selectedIndex].text.toUpperCase().includes('EFECTIVO'))hay=true;
    document.querySelectorAll('#contenedorPagos .pago-row').forEach(row=>{const sel=row.querySelector('select');const m=parseFloat(row.querySelector('input[type="number"]')?.value)||0;if(m>0&&sel&&sel.options[sel.selectedIndex].text.toUpperCase().includes('EFECTIVO'))hay=true;});
    const sv=document.getElementById('seccionVuelto');
    if(hay){sv.style.display='block';actualizarTotalAPagar();}else{sv.style.display='none';document.getElementById('pagaCon').value='';document.getElementById('vuelto').value='0.00';}
}
function actualizarTotalAPagar(){const m=parseFloat(document.getElementById('montoTotalFinal').value)||parseFloat(document.getElementById('montoTotal').value);document.getElementById('totalAPagar').value=m.toFixed(2);calcularVuelto();}
function calcularVuelto(){const t=parseFloat(document.getElementById('totalAPagar').value)||0;const p=parseFloat(document.getElementById('pagaCon').value)||0;const v=p-t;const el=document.getElementById('vuelto');if(v<0){el.value='0.00';el.className='form-control fw-bold text-danger';}else{el.value=v.toFixed(2);el.className='form-control fw-bold text-success';}}
function setPagaCon(m){const t=parseFloat(document.getElementById('totalAPagar').value)||0;document.getElementById('pagaCon').value=Math.max(m,t).toFixed(2);calcularVuelto();}

/* --- PAGO MODAL --- */
function initPagoModal() {
    let cP=1;
    document.getElementById('btnAgregarPago').addEventListener('click',()=>{
        const c=document.getElementById('contenedorPagos'); const d=document.createElement('div'); d.className='pago-row';
        d.innerHTML=`<div class="row g-2"><div class="col-md-5"><select class="form-control" name="formaPago_${cP}" onchange="detectarEfectivo()"><?php foreach(listarFormaPago_v2($sucursal_id)as$fp):?><option value="<?php echo $fp['id']?>"><?php echo $fp['nombre']?></option><?php endforeach;?></select></div><div class="col-md-5"><input type="number" class="form-control" name="monto_${cP}" placeholder="Monto S/" min="0" step="0.01" oninput="detectarEfectivo()"></div><div class="col-md-2"><button type="button" class="btn btn-sm btn-outline-danger rounded-pill w-100" onclick="this.closest('.pago-row').remove();detectarEfectivo();"><i class="fas fa-times"></i></button></div></div>`;
        c.appendChild(d); cP++;
    });
    let cC=1;
    document.getElementById('btnAgregarPagoCredito').addEventListener('click',()=>{
        const c=document.getElementById('contenedorPagosCredito'); const d=document.createElement('div'); d.className='pago-row';
        d.innerHTML=`<div class="row g-2"><div class="col-md-5"><select class="form-control" name="formaPagoCredito[]"><?php foreach(listarFormaPago_v2($sucursal_id)as$fp):?><option value="<?php echo $fp['id']?>"><?php echo $fp['nombre']?></option><?php endforeach;?></select></div><div class="col-md-5"><input type="number" class="form-control" name="montoCredito[]" placeholder="Monto S/" min="0" step="0.01"></div><div class="col-md-2"><button type="button" class="btn btn-sm btn-outline-danger rounded-pill w-100" onclick="this.closest('.pago-row').remove()"><i class="fas fa-times"></i></button></div></div>`;
        c.appendChild(d); cC++;
    });
}
...
async function fn_pagar_directo() {
    ...
    if(tipoC==='factura'&&idP==='#'){Swal.fire({icon:'error',title:'Error',text:'Para factura necesitas un cliente',timer:2000,showConfirmButton:false});return;}
    let fps=[],total=0,fp=null;
    fd.forEach(i=>{if(i.name.startsWith('formaPago')){fp=i.value;}else if(i.name.startsWith('monto')){const m=parseFloat(i.value);if(fp&&m>0){fps.push({id_forma_pago:fp,monto_forma_pago:m});total+=m;fp=null;}}});
    if(!fps.length){Swal.fire({icon:'error',title:'Error',text:'Agrega al menos una forma de pago',timer:2000,showConfirmButton:false});return;}
    if(Math.abs(total-montoF)>0.01){Swal.fire({icon:'error',title:'Error',text:'Los montos no coinciden con el total',timer:2000,showConfirmButton:false});return;}
    const sv=document.getElementById('seccionVuelto');
    if(sv.style.display!=='none'){
        ...
        if(vuelto<0){Swal.fire({icon:'warning',title:'Monto insuficiente',text:'El cliente debe pagar un monto mayor o igual al total',timer:2000,showConfirmButton:false});document.getElementById('pagaCon').focus();return;}
        if(vuelto>0){const r=await Swal.fire({title:'Confirmar Vuelto',html:`<div class="text-start"><p><b>Total:</b> S/ ${montoF.toFixed(2)}</p><p><b>Paga con:</b> S/ ${pagaCon.toFixed(2)}</p><p class="text-success fs-4"><b>Vuelto:</b> S/ ${vuelto.toFixed(2)}</p></div>`,icon:'question',showCancelButton:true,confirmButtonText:'Confirmar',cancelButtonText:'Cancelar',confirmButtonColor:'#11998e'});if(!r.isConfirmed)return;}
    }
    const venta={tipo_comprobante:tipoC,usuario_id:<?php echo $_SESSION['id']; ?>,cliente_id:idP==='#'?9897:parseInt(idP),monto_original:montoO,monto_venta_final:montoF};
    $.ajax({url:'logica/clssInsertPA.php',type:'POST',data:{accion:'FINALIZARVENTARAPIDO',jsDatosVenta:JSON.stringify(venta),js_articulos:JSON.stringify(obtenerArticulos()),js_detalle_pago:JSON.stringify(fps)},
    success:r=>{try{const res=JSON.parse(r);if(res.estado){Swal.fire({title:'Venta Exitosa',icon:'success',timer:1500,showConfirmButton:false}).then(()=>{window.open(`ticket.php?id=${res.id_venta_generado}`,'_blank');location.reload();});}else Swal.fire({icon:'error',title:'Error',text:res.mensaje});}catch(e){Swal.fire({icon:'error',title:'Error',text:'Error al procesar la respuesta'});}},
...
function fn_pagar_credito(){
    const idP=document.getElementById('idPersona').textContent.trim();
    if(idP==='#'){Swal.fire({icon:'error',title:'Error',text:'Debes seleccionar un cliente para venta a crédito',timer:2000,showConfirmButton:false});return;}
    const fd=$('#form-pago-credito').serializeArray();
    const montoO=parseFloat(document.getElementById('montoTotal').value);
    const montoF=parseFloat(document.getElementById('montoTotalFinal').value)||montoO;
    let dd=[],mi=0,fp=null;
    fd.forEach(i=>{if(i.name==='formaPagoCredito[]'){fp=i.value;}else if(i.name==='montoCredito[]'){const m=parseFloat(i.value)||0;if(fp&&m>0){dd.push({id_forma_pago:fp,monto_forma_pago:m});mi+=m;fp=null;}}});
    const venta={usuario_id:<?php echo $_SESSION['id']; ?>,cliente_id:parseInt(idP),monto_original:montoO,monto_venta_final:montoF,monto_inicial:mi};
    $.ajax({url:'logica/clssInsertPA.php',type:'POST',data:{accion:'FINALIZARVENTACREDITORAPIDO',jsDatosVenta:JSON.stringify(venta),js_articulos:JSON.stringify(obtenerArticulos()),js_detalle_deuda:dd.length>0?JSON.stringify(dd):null},
    success:r=>{try{const res=JSON.parse(r);if(res.estado){Swal.fire({title:'Crédito Registrado',icon:'success',timer:1500,showConfirmButton:false}).then(()=>{window.open(`ticket.php?id=${res.id_venta_generado}`,'_blank');location.reload();});}else Swal.fire({icon:'error',title:'Error',text:res.mensaje});}catch(e){Swal.fire({icon:'error',title:'Error',text:'Error al procesar la respuesta'});}},
    error:()=>Swal.fire({icon:'error',title:'Error',text:'Error en la comunicación con el servidor'})});
}
...
function buscarCliente(){const q=document.getElementById('nombreCliente').value.trim();const s=document.getElementById('sugerencias');if(!q.length){s.innerHTML='';return;}$.ajax({method:'POST',url:'logica/clssFiltro.php',data:{accion:'FILTROPERSONA',data:q,sucursal_id:<?php echo $sucursal_id;?>},success:r=>{try{const arr=JSON.parse(r);s.innerHTML='';arr.forEach(p=>{const item=document.createElement('div');item.className='list-group-item list-group-item-action';item.textContent=p.persona_concatenada;item.onclick=()=>{document.getElementById('nombreCliente').value=p.persona_concatenada;document.getElementById('idPersona').textContent=p.id;document.getElementById('idUpdateNumTelefonoCliente').value=p.telefonomovil||'';document.getElementById('idUpdateCorreoCliente').value=p.email||'';s.innerHTML='';};s.appendChild(item);});}catch(e){}}}); }
function registrarCliente(){const esP=document.getElementById('pills-persona-tab').classList.contains('active');if(esP){const dni=document.getElementById('numeroDocumentoPersona').value.trim();const nom=document.getElementById('nombresPersona').value.trim();const ape=document.getElementById('apellidosPersona').value.trim();if(!dni||!nom||!ape){Swal.fire({icon:'warning',title:'Campos requeridos',text:'Completa los campos obligatorios',timer:2000,showConfirmButton:false});return;}registrarPersona({numero_documento:dni,nombres:nom,apellidos:ape,telefono_movil:document.getElementById('telefonoPersona').value||null,email:document.getElementById('emailPersona').value||null});}else{const ruc=document.getElementById('numeroDocumentoEmpresa').value.trim();const rz=document.getElementById('razonSocial').value.trim();const nc=document.getElementById('nombreComercial').value.trim();if(!ruc||!rz||!nc){Swal.fire({icon:'warning',title:'Campos requeridos',text:'Completa los campos obligatorios',timer:2000,showConfirmButton:false});return;}registrarPersona({numero_documento:ruc,razon_social:rz,nombre_comercial:nc,telefono_movil:document.getElementById('telefonoEmpresa').value||null,email:document.getElementById('emailEmpresa').value||null});}}
function registrarPersona(datos){$.ajax({method:'POST',url:'logica/clssPersona.php',data:{accion:'REGISTRARPERSONARAPIDO',data:JSON.stringify(datos)},success:r=>{try{const res=JSON.parse(r);if(res.success){const id=res.persona_id||res.empresa_id;const nombre=datos.nombres?`${datos.numero_documento} - ${datos.nombres} ${datos.apellidos}`:`${datos.numero_documento} - ${datos.razon_social}`;document.getElementById('idPersona').textContent=id;document.getElementById('nombreCliente').value=nombre;bootstrap.Modal.getInstance(document.getElementById('modalCliente')).hide();Swal.fire({icon:'success',title:'Cliente registrado',timer:1500,showConfirmButton:false});}else Swal.fire({icon:'error',title:'Error',text:res.message});}catch(e){Swal.fire({icon:'error',title:'Error',text:'Error al procesar la respuesta'});}}}); }

function initEventListeners(){initSoloCorteModal();initImpresion3DModal();initPagoModal();initClienteModal();}
</script>
