...
<style>
    ...
    .page-header h3 { font-weight: 800; margin-bottom: 4px; }
    .page-header p { opacity: .85; margin-bottom: 0; font-size: .95rem; }
    ...
    /* ===== DATATABLE RESERVAS ===== */
    .tabla-reservas-wrap { overflow-x: auto; }
    ...
    #multi-filter-select tbody tr:hover { background: #f0f3ff; }
    ...
    .badge-estado { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: .75rem; font-weight: 700; }
    ...
    .btn-ver:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(102,126,234,.35); color: white; }
    ...
    .info-block { margin-bottom: 10px; font-size: .88rem; }
    .info-block .lbl { color: var(--text-muted); font-size: .75rem; font-weight: 600; text-transform: uppercase; letter-spacing: .5px; }
    .info-block .val { font-weight: 700; color: var(--primary); }
    ...
    #tabla_articulos th:nth-child(10), #tabla_articulos td:nth-child(10),
    #tabla_articulos th:nth-child(11), #tabla_articulos td:nth-child(11),
    #tabla_articulos th:nth-child(12), #tabla_articulos td:nth-child(12) {
    ...
    .resumen-stat .stat-lbl { font-size: .78rem; color: var(--text-muted); font-weight: 600; text-transform: uppercase; letter-spacing: .5px; }
    .resumen-stat .stat-val { font-size: 1.6rem; font-weight: 800; color: var(--primary); }
    .resumen-stat.total-card { background: var(--gradient-main); }
    .resumen-stat.total-card .stat-lbl { color: rgba(255,255,255,.8); }
    .resumen-stat.total-card .stat-val { color: white; }
    ...
    .panel-agregar-header h6 { font-weight: 800; margin: 0; }
    .panel-agregar-body { padding: 20px; overflow: visible; position: relative; }

    /* ===== SERVICIOS ===== */
    .servicios-wrap { display: flex; flex-wrap: wrap; gap: 7px; margin-bottom: 16px; }
    .btn-servicio {
        background: white;
    ...
    .btn-servicio:hover { background: var(--gradient-main); border-color: transparent; color: white; transform: translateY(-1px); box-shadow: 0 3px 10px rgba(102,126,234,.3); }
    ...
    .add-mode-tab.active { background: var(--gradient-main); border-color: transparent; color: white; box-shadow: 0 3px 10px rgba(102,126,234,.3); }
    ...
    #searchInput:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(102,126,234,.12); outline: none; background: white; }
    .search-icon-abs { position: absolute; right: 14px; top: 50%; transform: translateY(-50%); color: var(--accent); pointer-events: none; }
    ...
    #resultadosProductos::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 10px; }
    .producto-item {
        padding: 11px 16px;
        cursor: pointer;
    ...
    .prod-nombre { font-weight: 700; font-size: .85rem; color: #1a1a2e; line-height: 1.3; text-transform: uppercase; }
    .prod-info { font-size: .72rem; color: var(--text-muted); margin-top: 3px; }
    .prod-precio-badge {
        font-weight: 800;
        color: white;
        font-size: .88rem;
    ...
    .search-hint { font-size: .78rem; color: var(--text-muted); padding: 16px 0 4px; text-align: center; opacity: .7; }
    ...
    .form-manual .form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(102,126,234,.12); }
    ...
    .modal-body .input-group-text { border: 1.5px solid var(--border-soft); border-radius: 10px 0 0 10px; background: #f0f3ff; }
    ...
    /* ===== TABS MODAL PAGO ===== */
    .nav-pago .nav-link { border-radius: 10px; font-weight: 600; font-size: .88rem; color: var(--text-muted); padding: 8px 18px; transition: all .2s; border: none; }
    .nav-pago .nav-link.active { background: var(--gradient-main); color: white; box-shadow: 0 3px 10px rgba(102,126,234,.3); }
    .nav-pago .nav-link:not(.active):hover { background: #f0f3ff; color: var(--primary); }

    /* ===== FORMA PAGO ROW ===== */
    .pago-row { background: var(--bg-card); border: 1.5px solid var(--border-soft); border-radius: 12px; padding: 12px 16px; margin-bottom: 8px; }
    .pago-row select, .pago-row input { border: 1.5px solid var(--border-soft); border-radius: 10px; padding: 8px 12px; font-size: .88rem; }
    .pago-row select:focus, .pago-row input:focus { border-color: var(--accent); outline: none; }
    ...
    .imp-linea:last-child { border-bottom: none; }
    .imp-lbl { color: var(--text-muted); font-weight: 500; }
    .imp-val { font-weight: 700; color: var(--primary); }
    .imp-val.cero { color: #adb5bd; font-weight: 400; }
    .imp-linea.igv-line .imp-lbl { color: #b45309; font-weight: 700; }
    .imp-linea.igv-line .imp-val { color: #b45309; }
    .imp-linea.icbper-line .imp-lbl { color: #0891b2; }
    .imp-linea.icbper-line .imp-val { color: #0891b2; }
    .imp-linea.total-imp { background: var(--gradient-main); border-radius: 10px; padding: 8px 12px; margin-top: 6px; }
    .imp-linea.total-imp .imp-lbl { color: rgba(255,255,255,.85); font-weight: 700; font-size: .9rem; }
    .imp-linea.total-imp .imp-val { color: white; font-size: 1.05rem; }

    /* ===== VUELTO ===== */
    #seccionVuelto { background: #f0fff8; border: 1.5px solid #a7f3d0; border-radius: 14px; padding: 16px; margin-top: 14px; animation: slideDown .3s ease; }
    @keyframes slideDown { from { opacity: 0; transform: translateY(-14px); } to { opacity: 1; transform: translateY(0); } }
    .pago-rapido-btns { display: flex; gap: 6px; flex-wrap: wrap; justify-content: center; }
    .pago-rapido-btn { background: #e8f5e9; border: 1.5px solid #a7d7a8; border-radius: 16px; padding: 5px 14px; font-size: .82rem; font-weight: 700; color: var(--success); cursor: pointer; transition: all .2s; }
    .pago-rapido-btn:hover { background: var(--gradient-success); border-color: transparent; color: white; }

    /* ===== BOTONES GLOBALES ===== */
    .btn-success-custom { background: var(--gradient-success); border: none; color: white; border-radius: 12px; padding: 10px 24px; font-weight: 700; font-size: .92rem; transition: all .25s; box-shadow: 0 3px 12px rgba(17,153,142,.25); cursor: pointer; }
    .btn-success-custom:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(17,153,142,.35); color: white; }
    .btn-primary-custom { background: var(--gradient-main); border: none; color: white; border-radius: 12px; padding: 10px 24px; font-weight: 700; font-size: .92rem; transition: all .25s; cursor: pointer; }
    ...
    .fade-in-panel { animation: fadeInRow .35s ease; }
...
function agregarManual() {
    ...
    fn_adicionar_articulo(vid, datos).then(() => {
        ...
        fn_consultarVenta([{venta_id: vid}]);
...
function initSoloCorteModal() {
    document.getElementById('btnAbrirModalSolo').addEventListener('click', e => {
        ...
        new bootstrap.Modal(document.getElementById('modalSoloCorte'), {backdrop:'static'}).show();
        ...
        document.getElementById('btnInc05SC').onclick = () => { let p=parseFloat(document.getElementById('precioSoloCorte').value); document.getElementById('precioSoloCorte').value=(p+0.5).toFixed(2); };
        document.getElementById('btnInc1SC').onclick  = () => { let p=parseFloat(document.getElementById('precioSoloCorte').value); document.getElementById('precioSoloCorte').value=(p+1).toFixed(2); };
        document.getElementById('btnInc2SC').onclick  = () => { let p=parseFloat(document.getElementById('precioSoloCorte').value); document.getElementById('precioSoloCorte').value=(p+2).toFixed(2); };
        document.getElementById('btnInc5SC').onclick  = () => { let p=parseFloat(document.getElementById('precioSoloCorte').value); document.getElementById('precioSoloCorte').value=(p+5).toFixed(2); };
...
async function agregarDatosCorte() {
    ...
    if (min <= 0) { Swal.fire({icon:'warning',title:'Minutos inválidos',text:'Ingresa minutos mayores a 0',timer:2000,showConfirmButton:false}); return; }
    const d = {id:'0',minutos:min,costo_por_minuto:tar,costo:min*tar,articulo:'CORTE MATERIAL',id_movimiento:6,precio_venta:null,cantidad:null};
    const vid = document.getElementById('idVentaReserva').textContent;
    try {
...
function initImpresion3DModal() {
    document.getElementById('btnAbrirModalSolov2').addEventListener('click', e => {
        ...
        new bootstrap.Modal(document.getElementById('modalSoloCorteMaquina2'), {backdrop:'static'}).show();
    });
}
...
async function fn_agregar_impresion_a_tabla() {
    ...
    if (min <= 0) { Swal.fire({icon:'warning',title:'Minutos inválidos',text:'Ingresa minutos mayores a 0',timer:2000,showConfirmButton:false}); return; }
    const d = {id:'0',minutos:min,costo_por_minuto:tar,costo:min*tar,articulo:'MAQUINA DE IMPRESION 3D',id_movimiento:15,precio_venta:null,cantidad:null};
    const vid = document.getElementById('idVentaReserva').textContent;
    try {
        await fn_adicionar_articulo(vid, d);
        document.getElementById('cantidad_solocortev2').value = 10;
...
function fnAumentoOrResta(a) { let v=parseFloat(document.getElementById('cantidad_solocortev2').value); document.getElementById('cantidad_solocortev2').value=a==='+'?v+1:(v>1?v-1:v); }
...
function fn_agregar_venta(articulo) {
    ...
    document.getElementById('btnRCAG').onclick = () => { let v=parseInt(document.getElementById('inpCAG').value); if(v>1){document.getElementById('inpCAG').value=v-1; if(v-1===1&&articulo.corte) document.getElementById('secCAG').style.display='block'; else if(v-1>1) document.getElementById('secCAG').style.display='none';} };
    document.getElementById('btnSCAG').onclick = () => { let c=parseInt(document.getElementById('inpCAG').value); if(c+1>articulo.stock){Swal.fire({icon:'warning',title:'Stock insuficiente',text:`Máx ${articulo.stock}`,timer:2000,showConfirmButton:false});return;} document.getElementById('inpCAG').value=c+1; if(c+1>1) document.getElementById('secCAG').style.display='none'; };
    document.getElementById('btnRMAG').onclick = () => { let v=parseInt(document.getElementById('inpMAG').value); if(v>0) document.getElementById('inpMAG').value=v-1; };
    document.getElementById('btnSMAG').onclick = () => { let v=parseInt(document.getElementById('inpMAG').value); document.getElementById('inpMAG').value=v===0?10:v+1; };
    ...
    document.getElementById('btnConfAG').addEventListener('click', async () => {
        ...
        if (cant > articulo.stock) { Swal.fire({icon:'warning',title:'Stock insuficiente',text:`Máx ${articulo.stock}`,timer:2000,showConfirmButton:false}); return; }
        if (articulo.corte && cant===1 && min<=0) { Swal.fire({icon:'warning',title:'Faltan minutos',text:'Ingresa minutos de corte',timer:2000,showConfirmButton:false}); return; }
        ...
        try {
            ...
            document.getElementById('resultadosProductos').style.display = 'none';
            document.getElementById('searchInput').value = '';
            const hint = document.getElementById('searchHint'); if(hint) hint.style.display='block';
            fn_consultarVenta([{venta_id: vid}]);
        } catch(e) { console.error(e); }
    });
}
...
function ajustarCantidadSv(s, inc) { const i=document.getElementById(`cant_${s}`); i.value=Math.max(1,parseInt(i.value)+inc); }
function agregarServicioSv(nombre, idMov) {
    ...
    if (monto<=0) { Swal.fire({icon:'warning',title:'Error',text:'Monto inválido',timer:2000,showConfirmButton:false}); return; }
    const vid = document.getElementById('idVentaReserva').textContent;
...
function fn_consultarVenta(datosArticulo) {
    ...
    $.ajax({
        ...
        data: {accion:'CONSULTARRESERVA', venta_id}
    }).done(function(text) {
        ...
        setTimeout(() => document.getElementById('panelDetalles').scrollIntoView({behavior:'smooth'}), 200);
    });
}
...
function fn_agregar_articulo_tabla(d) {
    ...
    btnEl.addEventListener('click', async () => {
        const r = await Swal.fire({title:'¿Eliminar?',text:'Esta acción no se puede deshacer.',icon:'warning',showCancelButton:true,confirmButtonColor:'#dc3545',cancelButtonColor:'#6c757d',confirmButtonText:'Sí',cancelButtonText:'Cancelar'});
        if (r.isConfirmed) {
            const vid = document.getElementById('idVentaReserva').textContent;
...
function fnEditarItem(d, fila) {
    ...
    if (d.movimiento_id == 1) {
        ...
        m.show();
        document.getElementById('btnRCE').onclick = () => { let v=parseInt(document.getElementById('inpCE').value); if(v>1) document.getElementById('inpCE').value=v-1; };
        document.getElementById('btnSCE').onclick = () => { document.getElementById('inpCE').value=parseInt(document.getElementById('inpCE').value)+1; };
        document.getElementById('btnRKE').onclick = () => { let v=parseInt(document.getElementById('inpME').value); if(v>0) document.getElementById('inpME').value=v-1; };
        document.getElementById('btnSKE').onclick = () => { let v=parseInt(document.getElementById('inpME').value); document.getElementById('inpME').value=v===0?10:v+1; };
        document.getElementById('btnConfEd').addEventListener('click', async () => {
            d.cantidad = parseInt(document.getElementById('inpCE').value);
            if (d.corte) { d.minutos=parseInt(document.getElementById('inpME').value)||0; d.costo_por_minuto=parseFloat(document.getElementById('inpPE').value)||0; }
        ...
        const modal = new bootstrap.Modal(document.getElementById('modalSoloCorte'),{backdrop:'static'});
        modal.show();
        document.getElementById('btnRSCE').onclick = () => { let v=parseInt(document.getElementById('cantidad_solocorte').value); if(v>0) document.getElementById('cantidad_solocorte').value=v-1; };
        document.getElementById('btnSSCE').onclick = () => { document.getElementById('cantidad_solocorte').value=parseInt(document.getElementById('cantidad_solocorte').value)+1; };
...
function calcularImpuestos() {
    ...
    let opGravadas=0, montoIGV=0, opExoneradas=0, opInafectas=0, montoICBPER=0;
    ...
    return { gravadas: opGravadas, igv: montoIGV, exoneradas: opExoneradas, inafectas: opInafectas, icbper: montoICBPER, total };
}
...
function detectarEfectivo() {
    ...
    if (hay) { document.getElementById('seccionVuelto').style.display = 'block'; actualizarTotalAPagar(); }
    else      { document.getElementById('seccionVuelto').style.display = 'none'; document.getElementById('pagaCon').value = ''; document.getElementById('vuelto').value = '0.00'; }
}
...
function calcularVuelto() {
    ...
    if (v < 0) { el.value = '0.00'; el.className = 'form-control fw-bold text-danger'; }
    else        { el.value = v.toFixed(2); el.className = 'form-control fw-bold text-success'; }
}
...
async function fn_pagar_directo() {
    ...
    let radioSel;
    radios.forEach(r => { if (r.checked) radioSel = r.value; });
    ...
    if (sv.style.display !== 'none') {
        ...
        if (!pagaCon) { Swal.fire({icon:'warning',title:'Atención',text:'Ingresa el monto con que paga el cliente',timer:2000,showConfirmButton:false}); document.getElementById('pagaCon').focus(); return; }
        if (vuelto < 0) { Swal.fire({icon:'warning',title:'Monto insuficiente',text:'El cliente debe pagar un monto mayor o igual al total',timer:2000,showConfirmButton:false}); return; }
        if (vuelto > 0) {
            const r = await Swal.fire({title:'Confirmar Vuelto',html:`<p><b>Total:</b> S/ ${montoFinal.toFixed(2)}</p><p><b>Paga con:</b> S/ ${pagaCon.toFixed(2)}</p><p class="text-success fs-4"><b>Vuelto:</b> S/ ${vuelto.toFixed(2)}</p>`,icon:'question',showCancelButton:true,confirmButtonText:'Confirmar',cancelButtonText:'Cancelar',confirmButtonColor:'#11998e'});
            if (!r.isConfirmed) return;
        }
    }

    let jsPagos=[]; let fp=null, m=null, acum=0;
    datos.forEach(d => {
        if (d.name.startsWith('formaPago')) fp = d.value;
        if (d.name.startsWith('monto')) { m = parseFloat(d.value); acum += m; }
        if (fp && m) { jsPagos.push({venta_id:idVenta, id_forma_pago:fp, monto_forma_pago:m}); fp=null; m=null; }
    });
    if (radioSel==='factura' && !document.getElementById('idRucReceptor').value.trim()) { Swal.fire({icon:'error',title:'Falta RUC',text:'Ingresa RUC y Razón Social para factura'}); return; }
    if (!jsPagos.length) { Swal.fire({icon:'error',title:'Sin montos',text:'Agrega al menos una forma de pago'}); return; }
    if (acum > montoFinal) { Swal.fire({icon:'error',title:'Montos incorrectos',text:'Los montos superan el total'}); return; }
    if (acum < montoFinal) { Swal.fire({icon:'error',title:'Montos incorrectos',text:'Los montos son menores al total'}); return; }
    ...
    $.ajax({
        ...
        data: {
            ...
            jsDatosVenta: JSON.stringify({
                ...
                js_detalles_receptor_factura: {ruc:document.getElementById('idRucReceptor').value, razon_social:document.getElementById('idRazonSocialReceptor').value},
                venta_id: idVenta,
        ...
        success: r => {
            try {
                ...
                if (res.estado === true) Swal.fire({title:'¡Pagado!',text:res.mensaje,icon:'success',timer:1500,showConfirmButton:false}).then(() => { window.open('ticket.php?id='+parseInt(idVenta),'_blank'); location.reload(); });
                else Swal.fire({icon:'error',title:'Error',text:res.mensaje});
            } catch(e) {}
        },
        error: () => Swal.fire({icon:'error',title:'Error',text:'Error en la comunicación'})
    });
}
...
function fn_pagar_credito() {
    ...
    let jsDeuda=[]; let fp=null, m=null, acum=0;
    datos.forEach(d => {
        if (d.name.startsWith('formaPagoCredito')) fp = d.value;
        if (d.name.startsWith('montoCredito'))   { m=parseFloat(d.value); acum+=m; }
        if (fp && m) { jsDeuda.push({venta_id:idVenta, id_forma_pago:fp, monto_forma_pago:m}); fp=null; m=null; }
    });
    ...
    $.ajax({
        ...
        success: r => {
            try {
                const res = JSON.parse(r);
                if (res.estado === true) Swal.fire({title:'¡Crédito registrado!',text:res.mensaje,icon:'success',timer:1500,showConfirmButton:false}).then(() => { window.open('ticket.php?id='+parseInt(idVenta),'_blank'); location.reload(); });
                else Swal.fire({icon:'error',title:'Error',text:res.mensaje});
            } catch(e) {}
        },
        error: () => Swal.fire({icon:'error',title:'Error',text:'Error en la comunicación'})
...
function fn_insert_movimiento(venta_id, movimiento_id, cantidad, nota_archivo, sub_total) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'INSERTMOVIMIENTO',data:JSON.stringify({venta_id,movimiento_id,cantidad,sub_total,nota_archivo})}})
        .done(r => { const j=JSON.parse(r); if(j.success){showNotification&&showNotification('success');fn_consultarVenta([{venta_id}]);resolve(j);}else reject(new Error(j.mensaje)); })
        .fail(e => reject(e));
    });
}
...
function fn_adicionar_articulo(venta_id, d) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'ADICIONARARTICULO',data:JSON.stringify({venta_id,articulo_id:d.id,cantidad:d.cantidad,sub_total:calcularSubTotal(d),minutos:d.minutos,precio_unitario:d.precio_venta,costoxminuto:d.costo_por_minuto,movimiento_id:d.id_movimiento})}})
        .done(r => { const j=JSON.parse(r); if(j.success){showNotification&&showNotification('success');resolve(j);}else{showNotification&&showNotification('error');Swal.fire({icon:'error',title:'Error',text:j.message});reject(new Error(j.message));} })
        .fail(e => reject(e));
    });
}
...
function fn_eliminar_articulo(id) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'ELIMINARARTICULO',id_rel_articulo:id}})
        .done(r => { const j=JSON.parse(r); j.success?resolve(j):reject(new Error(j.mensaje)); })
        .fail(e => reject(e));
    });
}
...
function fn_eliminar_movimiento(id) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'ELIMINARMOVIMIENTO',id_rel_articulo:id}})
        .done(r => { const j=JSON.parse(r); j.success?resolve(j):reject(new Error(j.mensaje)); })
        .fail(e => reject(e));
    });
}
...
function fn_editar_articulo(datos) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'EDITARARTICULO',data:JSON.stringify(datos)}})
        .done(r => { const j=JSON.parse(r); j.success?resolve(j):reject(new Error(j.mensaje)); })
        .fail(e => reject(e));
    });
}
...
function fn_editar_movimiento(datos) {
    return new Promise((resolve, reject) => {
        $.ajax({method:'POST',url:'logica/clssVentaCorte.php',data:{accion:'EDITARMOVIMIENTO',data:JSON.stringify(datos)}})
        .done(r => { const j=JSON.parse(r); j.success?resolve(j):reject(new Error(j.mensaje)); })
        .fail(e => reject(e));
    });
}
